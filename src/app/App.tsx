...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-60 flex flex-col bg-white transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        style={{ borderRight: "1px solid #e5e7eb" }}>

        {/* Brand */}
        <div className="flex items-center justify-between h-16 px-5 flex-shrink-0" style={{ borderBottom: "1px solid #e5e7eb" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
              style={{ background: "linear-gradient(135deg, #1f6fff 0%, #4f8dff 100%)" }}>
              <Shield size={15} className="text-white" />
            </div>
            <div>
              <p className="font-bold text-gray-900 text-sm leading-tight">FrotaPM</p>
              <p className="text-[11px] text-gray-500 leading-tight">Blumenau - SC</p>
            </div>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
          {sections.map((section) => (
            <div key={section ?? "root"} className="mb-4">
              {section && (
                <p className="text-[10px] text-gray-400 uppercase tracking-widest px-3 mb-1.5 font-semibold">{section}</p>
              )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Top bar */}
        <header className="h-14 bg-white flex items-center px-5 gap-4 flex-shrink-0" style={{ borderBottom: "1px solid #e5e7eb" }}>
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            <button className="relative p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
              <Bell size={18} />
              <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-blue-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center leading-none">0</span>
            </button>
